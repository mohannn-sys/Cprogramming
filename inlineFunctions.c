/*The main advantage of using inline functions is that it can lead to potentially faster code execution by avoiding the overhead associated with a regular function call. Instead of jumping to a separate function, the code of the inline function is inserted directly at the point where the function is called, reducing the function call overhead./*
n inline function is a function that is expanded in place at the point where it is called, rather than being called through a regular function call mechanism.
